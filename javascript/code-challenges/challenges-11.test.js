...
Write a function that, given an array of integer arrays as input, calculates the 
total sum of all the elements in the array.

You may want to use filter, map, or reduce for this problem, but are not required
 to. You may need to use the same method more than once.
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newARRay=[];
  element.filter(item =>{
  if(typeof item=='number'&&item %5==0){newARRay.push(2**item)}
...
returns the names of the characters whose gender 
